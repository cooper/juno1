...
sub sendsplash {
  ...
  $this->sendnumeric($this->server,1,
   "Welcome to \2".$this->server->{network}."\2, ".$this->nick."!");
  $this->sendnumeric($this->server,2,
   "Your host is ".$this->server->{name}.", running version pircd-juno");
  ...
  if($excess[1]) {
	...
	if(scalar keys(%{$user->{channels}})>0) {
	  foreach my $channel (values(%{$user->{channels}})) {
		...
		$tmpstr = "~".$tmpstr;
	      } elsif($channel->isadmin($user)) {
		$tmpstr = "&".$tmpstr;
	      } elsif($channel->isop($user)) {
		$tmpstr = "@".$tmpstr;
	      } elsif($channel->ishalfop($user)) {
		$tmpstr = "%".$tmpstr;
	...
	if ($this->ismode("o")) {
	  $this->sendnumeric($this->server,378,($user->nick, "is connected from *@".$user->host." ".$user->ip));
	}
...
sub handle_who {
  ...
  foreach $target (split(/,/,$targets)) {
	...
	unless(($ret->ismode('s'))&&(!defined($ret->{users}->{$this->nick()}))&&(!$this->ismode('S'))) {
	  my @users = $ret->users();
...
sub handle_kline {
  # most modern ircds allow you to kline users directly from the ircd
  # this is possible with juno as of june 28, 2010 :)
  my($this,$c) = (shift,shift);
	if ($this->ismode('o')) {
		my @ex = split(" ",$c);
		if ($ex[1]) {
			if ($ex[1] =~ m/\@/) {
				...
				if (!defined($banned)) {
				open CONF, ">>server.conf" or $this->privmsgnotice("NOTICE",$this->server,"Error writing to config file: $!");
				print CONF "K:$ident:$host:$reason\n";
				close CONF;
   				$this->sendnumeric($this->server,382,("server.conf"),"Rehashing");
   				$this->server->rehash();
				$this->server->opernotify($this->nick." added a kline on $ident\@$host [$reason]");
				$this->privmsgnotice("NOTICE",$this->server,"Kill line added: $ident\@$host :$reason");
				foreach my $usr (values(%{$this->server->{'users'}})) { # Kill the current connected users
					foreach my $ref (@{$this->server->{'klines'}}) {
					my $mask = $ref->[2];
						if($usr->{'host'} =~ /$mask/) {
						my @kline = @{$ref};
							if($usr->{'user'} =~ /$kline[1]/) {
      								$usr->sendnumeric($this->server,465,"*** Banned");
      								$usr->senddata("ERROR :Closing Link: (".$usr->{'host'}.") [K-lined: $reason]\r\n");
								$usr->quit("K-lined");
							}
						}
					}
					...
					# if the kline exists, they must be attempting to remove it.
					...
					close CONF;
					open WRITE, ">server.conf" or $this->privmsgnotice("NOTICE",$this->server,"Error writing to config file: $!");
					Utils::plog("lred","Error","Could not write to config: $!");
					print WRITE $current;
					close WRITE;
   				$this->sendnumeric($this->server,382,("server.conf"),"Rehashing");
   				$this->server->rehash();
				$this->server->opernotify($this->nick." removed kline on $ident\@$host [$breason]");
				$this->privmsgnotice("NOTICE",$this->server,"Kill line removed: $ident\@$host");
				}
			}
			else {
				$this->privmsgnotice("NOTICE",$this->server,"kline: must be ident\@host mask.");
			}
		}
		else {
			$this->sendnumeric($this->server,461,"kline: Not enough parameters");
		}
	}
	else {
...
sub handle_motd {
  ...
  $this->sendnumeric($this->server,376,"End of message of the day.");
}
...
sub handle_map {
  ...
  $this->sendnumeric($this->server,7,"End of server map");
}
	...
	if($byte eq "+") {
	  ...
	  if($state) {
	    ...
	    if($this->unsetmode($byte)) {
	      ...
	      if($byte eq 'o') {
		push(@accomplishedunset,'S') if $this->unsetmode('S');
...
sub handle_oper {
  ...
  if(defined($opers->{$nick})) {
	...
	$this->sendnumeric($this->server,464,"Password Incorrect.");
      ...
      $this->sendnumeric($this->server,491,"No O-lines for your host.");
    }
  } else {
...
sub handle_cap {
  # we don't really do anything with "CAP END", so there's no need to make this more complicated than it needs to be :P
}
#####################################################################
...
sub setmode {
  ...
  if(!&isvalidusermode($mode)) {
    $this->senddata(":".$this->server->{name}." 501 ".$this->nick." :Unknown mode \2$mode\2\r\n");
    return 0;
  }
  if(!$this->{modes}->{$mode}) {
    $this->{modes}->{$mode} = 1;
...
sub unsetmode {
  ...
  my $mode = shift;
  if(!&isvalidusermode($mode)) {
    $this->senddata(":".$this->server->{name}." 501 ".$this->nick." :Unknown mode flag \2$mode\2\r\n");
    return 0;
  }
  if($this->{modes}->{$mode}) {
...
sub privmsgnotice {
  ...
  if($this->{'socket'}) {
# TODO send to servers in which involve the client (whether it's privmsging the client
# or privmsging a channel with a client on that server involved.
# this is gonna be so hard... I don't even know where to begin
   if(isa($from,"User")) {
#	foreach my $server (Utils::servers) {
#		$svr = Utils::lookup($server->{name});
#		$tsrv = Utils::lookup($this->server->name);
#		$tsrv->
#	}
...
sub kill {
  ...
  multisend(":$$this{'nick'}!$$this{'user'}\@$$this{'host'} QUIT>:Killed (".$from->nick." \($excuse\))",@foo);
